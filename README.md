...
| <a name="input_droplet_image"></a> [droplet\_image](#input\_droplet\_image) | The image slug of the droplet (must be available in the region). Default: 'ubuntu-22-04-x64' from public catalog | `string` | `"ubuntu-22-04-x64"` | no |
| <a name="input_droplet_image_id"></a> [droplet\_image\_id](#input\_droplet\_image\_id) | The image ID of the droplet (if you want to use a specific image ID instead of automatic resolving from the image slug). Conflicts with `droplet_image`, this one has higher priority. Default: null | `string` | `null` | no |
