...
| <a name="input_droplet_image_id"></a> [droplet\_image\_id](#input\_droplet\_image\_id) | The image ID of the droplet (if you want to use a specific image ID instead of automatic resolving from the image slug). Takes priority over `droplet_image` when provided. Default: null | `string` | `null` | no |
